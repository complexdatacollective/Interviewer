/*global extend, Kinetic, notify, network, session, tinycolor */
/* exported altCanvas */
/*jshint bitwise: false*/

'use strict';

var altCanvas = function altCanvas(userSettings) {
	...
	var energising = false;
	var energy = 60;
	var minEnergy = 35;
	var energyStroke = 20;
	var angularSpeed = 360 / energy;
	...
	function narrativeTween(object) {
		console.log(object);
		var colorTween = new Kinetic.Tween({
			...
			onFinish: function() {
				setTimeout(function() {
					var reverseColorTween = new Kinetic.Tween({
						node: object.children[0],
						duration: 15,
						fillRed: 1,
						fillGreen: 0,
		...
		var groupTween = new Kinetic.Tween({
			node: object,
			duration: 0.3,
			scaleX: 1.5,
			scaleY: 1.5,
			...
			onFinish: function() {
				setTimeout(function() {
					var reverseGroupTween = new Kinetic.Tween({
						node: object,
						duration: 15,
						scaleX: 1,
						scaleY: 1,
	...
	function energiseNode(object) {
		console.log(object);
		console.log('energise node');
		// object.children[0].dash([5, 5]);

		var timer;
		timer = setInterval(function() {
			// console.log('timer');
			if (energising === true) {
				// console.log('energising...');
				if (object.attrs.energy >= minEnergy) {
					object.attrs.energy = object.attrs.energy - 1;
					// console.log(object.attrs.energy);
					angularSpeed = 360/object.attrs.energy;
					var anim = new Kinetic.Animation(function(frame) {
						// console.log(frame.timeDiff);
						// time diff is the time since the last animation frame.
						// var angleDiff = frame.timeDiff * angularSpeed / 1000;
						// console.log(angleDiff);
						// energy is 60 and decreases to be as low as 20.
						// range is energy - min energy.
						// current energy is value within this range
						// high limit = (60-20) / 40 = 1
						var offset = ((energy-object.attrs.energy)/(energy-minEnergy));
						// console.log(offset);
						// var amount = energy-object.attrs.energy;
						var color = tinycolor.mix('#221100', '#ffe655', (offset*100));
						// console.log(15 * Math.sin(frame.time * 2 * Math.PI / 2000)+15);
						object.children[0].fill(color);
						object.children[0].stroke(color);
						var stroke = energyStroke/2;
						object.children[0].strokeWidth(((stroke*offset)) * Math.sin(frame.time * 2 * Math.PI / ((1-offset)*4000))+(stroke*offset)+5);
						// object.children[0].rotate(angleDiff);
					}, nodeLayer);
	...
	canvas.init = function () {
		notify('altCanvas initialising.', 1);
		...
		//   	canvasMenu = menu.addMenu('altCanvas','hi-icon-support');
		//
    	...
    	// Iterate over them
  		...
  		} else if (settings.mode === 'Select' || settings.mode === 'Narrative' || settings.mode === 'Energy') {
  			// Select mode
	...
	canvas.addNode = function(options) {
		notify('altCanvas is creating a node.',2);
		...
		nodeGroup.on('touchstart', function() {
		...
		nodeGroup.on('touchend', function() {
		...
		nodeGroup.on('tap click', function() {
				...
				if (settings.variable) {
					...
					if(selectedNodes.length === 2) {
						...
						if (network.addEdge(edgeProperties) === false) {
							notify('altCanvas removing edge.',2);
			...
			} else if (settings.mode === 'Narrative') {
				narrativeTween(this);
			}
			this.moveToTop();
			nodeLayer.draw();
	...
	canvas.addEdge = function(properties) {
		...
		// var toObject = network.getNode(properties.to);
			notify('altCanvas is adding an edge.',2);
