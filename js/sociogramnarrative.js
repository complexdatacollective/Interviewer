...
module.exports = function sociogramNarrative() {
	...
	sociogramNarrative.hullListClickHandler = function(e) {
		...
		console.log(sociogramNarrative.selectedNode);
		if (sociogramNarrative.selectedNode.attrs.contexts.indexOf(selectedHull) !== -1 ) {
			clicked.removeClass('active');
			sociogramNarrative.removePointFromHull(sociogramNarrative.selectedNode, selectedHull);
		} else {
			clicked.addClass('active');
			sociogramNarrative.addPointToHull(sociogramNarrative.selectedNode, selectedHull);
		}
	};

	function groupButtonClickHandler() {
		sociogramNarrative.addHull();
	}
	...
	sociogramNarrative.bindEvents = function() {
		...
		var event = [
			...
			{
				event: 'click',
				handler: sociogramNarrative.showNewNodeForm,
				targetEl:  '.new-node-button'
			}, {
				event: 'click',
				handler: sociogramNarrative.hullListClickHandler,
				targetEl:  window.document,
	...
	sociogramNarrative.timeSelectedNode = function() {
	...
	sociogramNarrative.removePointFromHull = function(point, hullLabel) {
		note.info('sociogramNarrative.removePointFromHull()');
		var properties;
		...
		if (settings.dataOrigin.Community.type === 'node') {
			...
			if (point.attrs[settings.dataDestination.Community.variable] === 'undefined') {
				note.error('sociogramNarrative.removePointFromHull(): Error! The point wasn\'t attached to a hull named '+hullLabel);
				return false;
			}
	...
	sociogramNarrative.addNode = function(options) {
		...
		nodeGroup.on('longPress', function() {
			...
			sociogramNarrative.showDetailsPanel();
			console.log('!!!setting selected node: longpress')
			sociogramNarrative.selectedNode = this;
			console.log(sociogramNarrative.selectedNode);
			var currentNode = this;
	...
	sociogramNarrative.showNewNodeForm = function() {
	...
	sociogramNarrative.getSimpleNodes = function() {
		// We need to create a simple representation of the nodes for storing.
		var simpleNodes = {};
		var nodes = sociogramNarrative.getKineticNodes();
	...
	sociogramNarrative.getSimpleEdges = function() {
		var simpleEdges = {},
		edgeCounter = 0;
	...
	sociogramNarrative.getSimpleEdge = function(id) {
		var simpleEdges = sociogramNarrative.getSimpleEdges();
		if (!id) { return false; }
