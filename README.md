...
- Native dependencies won't compile
  ...
  - ...You could install python and VS Build Tools manually; you should _not_ need all of Visual Studio
...
_Known issue & caveat_: this requires temporarily changing `config.xml` contents to build a development app; if everything goes well, the changes are cleaned up upon completion. However, a Cordova build error (for example) can leave config.xml in this 'development' state.
