...
var RoleRevisit = function RoleRevisit() {
    ...
    var cardClickHandler = function(e) {
        ...
        global.network.removeEdges(global.network.getEdges({from:global.network.getNodes({type_t0:'Ego'})[0].id, to: editing, type:'Role'}));
    ...
    roleRevisit.destroy = function() {
        global.tools.notify('Destroying roleRevisit.',0);
        // Event listeners
    ...
    roleRevisit.init = function(options) {
        global.tools.extend(roleRevisit.options, options);
        // create elements
