...
expect.extend({
  toMatchCoords(received, coords) {
    ...
    if (passX && passY) {
      return {
        message: () =>
          `expected ${JSON.stringify(received)} not to match ${JSON.stringify(coords)}`,
        pass: true,
      };
    }
    ...
    return {
      message: () =>
        `expected ${JSON.stringify(received)} to match ${JSON.stringify(coords)}`,
      pass: false,
    };
  },
});
...
describe('useViewport', () => {
  ...
  it('returns correct interface', () => {
    const subject = mount(<TestConsumer />);
  ...
  describe('navigation', () => {
    it('correctly moves viewport', () => {
      ...
      const { viewport, moveViewport, zoomViewport } = subject
        .find('div')
        .prop('useViewport');
  ...
  describe('calculateLayoutCoords()', () => {
    it('correctly calculates layout coords for default viewport', () => {
      ...
      const { calculateLayoutCoords } = subject.find('div').prop('useViewport');

      // layout_space = -500, 500
      // zoom = 3;

      expect(calculateLayoutCoords({ x: 0, y: 0 })).toMatchCoords({
        x: -500 / 3,
        y: -500 / 3,
      });
      expect(calculateLayoutCoords({ x: 0.5, y: 0 })).toMatchCoords({
        x: 0,
        y: -500 / 3,
      });
      expect(calculateLayoutCoords({ x: 1, y: 0.5 })).toMatchCoords({
        x: 500 / 3,
        y: 0,
      });
      expect(calculateLayoutCoords({ x: 1, y: 1 })).toMatchCoords({
        x: 500 / 3,
        y: 500 / 3,
      });
    });
    ...
    it('correctly calculates layout coords for modified viewport', () => {
      ...
      const { calculateLayoutCoords, moveViewport, zoomViewport } = subject
        .find('div')
        .prop('useViewport');
      ...
      expect(calculateLayoutCoords({ x: 0.5, y: 0.5 })).toMatchCoords({
        x: 100,
        y: 100,
      });
      expect(calculateLayoutCoords({ x: 1, y: 1 })).toMatchCoords({
  ...
  describe('calculateRelativeCoords()', () => {
    it('correctly calculates relative coords for default viewport', () => {
      ...
      const { calculateRelativeCoords } = subject
        .find('div')
        .prop('useViewport');

      expect(calculateRelativeCoords({ x: -500, y: -500 })).toMatchCoords({
        x: 0,
        y: 0,
      });
      expect(calculateRelativeCoords({ x: 0, y: -500 })).toMatchCoords({
        x: 0.5,
        y: 0,
      });
      expect(calculateRelativeCoords({ x: 500, y: 0 })).toMatchCoords({
        x: 1,
        y: 0.5,
      });
      expect(calculateRelativeCoords({ x: 500, y: 500 })).toMatchCoords({
    ...
    it('correctly calculates relative coords for moved viewport', () => {
      ...
      const { calculateRelativeCoords, moveViewport } = subject
        .find('div')
        .prop('useViewport');

      moveViewport(0.1, 0.1, true); // center = 100, 100

      expect(calculateRelativeCoords({ x: -400, y: -400 })).toMatchCoords({
        x: 0,
        y: 0,
      });
      expect(calculateRelativeCoords({ x: 100, y: 100 })).toMatchCoords({
        x: 0.5,
        y: 0.5,
      });
      expect(calculateRelativeCoords({ x: 600, y: 600 })).toMatchCoords({
        x: 1,
        y: 1,
      });
    });
    ...
    it('correctly calculates relative coords for zoomed viewport', () => {
      ...
      const { calculateRelativeCoords, zoomViewport } = subject
        .find('div')
        .prop('useViewport');

      zoomViewport(2, true); // zoom = 2

      expect(calculateRelativeCoords({ x: -250, y: -250 })).toMatchCoords({
        x: 0,
        y: 0,
      });
      expect(calculateRelativeCoords({ x: 0, y: 0 })).toMatchCoords({
        x: 0.5,
        y: 0.5,
      });
      expect(calculateRelativeCoords({ x: 250, y: 250 })).toMatchCoords({
    ...
    it('correctly calculates relative coords for moved and zoomed viewport', () => {
      ...
      const { calculateRelativeCoords, moveViewport, zoomViewport } = subject
        .find('div')
        .prop('useViewport');
      ...
      expect(calculateRelativeCoords({ x: -150, y: -150 })).toMatchCoords({
        x: 0,
        y: 0,
      });
      expect(calculateRelativeCoords({ x: 100, y: 100 })).toMatchCoords({
      ...
      expect(calculateRelativeCoords({ x: 350, y: 350 })).toMatchCoords({
        x: 1,
        y: 1,
      });
    });
