...
describe('useSort', () => {
  it('returns correct interface', () => {
    const subject = mount(
      <TestConsumer list={mockList} initialSortOrder={sortOrder} />,
    );
  ...
  it('can toggle direction', () => {
    ...
    const { sortedList: reversedSortedList, sortDirection } = subject
      .find('div')
      .prop('useSort');
  ...
  it('can change sort property', () => {
    ...
    const { updateSortByProperty } = subject.find('div').prop('useSort');

    const { sortDirection: changedSortDirection } = subject
      .find('div')
      .prop('useSort');
