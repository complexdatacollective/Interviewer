...
const checkExistingSession = (currentName) => {
  ...
  if (unExportedSession) {
    const message =
      'This protocol is already installed, and in-progress sessions using it have not yet been exported. Please export or delete these sessions before attempting to overwrite the protocol.';
    return Promise.reject(message);
  }

  const existingSession = findKey(
    state.sessions,
    ...
    return new Promise((resolve, reject) =>
      dispatch(
        dialogActions.openDialog({
          ...
          onCancel: () =>
            reject(
              new CancellationError('Installation of this protocol cancelled.'),
            ),
          message:
            'This protocol is already installed. Overwriting the previous installation of this protocol may prevent you from modifying previously created sessions.',
        }),
      ),
    );
  }
...
export const moveToExistingProtocol = (existingIndex, protocolContent) => {
  return renameProtocol(existingIndex, protocolContent.uid).then(
    () => protocolContent,
  );
};
