...
const StagesMenu = (props) => {
  const { currentStageIndex, stages, setExpanded, onStageSelect } = props;
  ...
  const calculateDelay = (currentItemIndex) => {
    ...
    if (currentStageIndex === 0 && currentItemIndex < 8) {
  ...
  const filteredStageList = stages.filter((stage) =>
  ...
  }, [currentStageIndex, filter, scrollToLocation]);
  ...
  return (
    <motion.div
      ...
      layout
    >
      <motion.article className="stages-menu__wrapper">
        {renderMenuItems.length > 0 ? (
          <Scroller useSmoothScrolling={false} ref={scrollerRef}>
            <AnimateSharedLayout>
              <AnimatePresence>{renderMenuItems}</AnimatePresence>
            </AnimateSharedLayout>
          </Scroller>
        ) : (
...
export default compose(connect(mapStateToProps, null))(StagesMenu);
