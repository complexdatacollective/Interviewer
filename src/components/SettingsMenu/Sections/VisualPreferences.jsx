    ...
    if (isElectron() && window.electronAPI?.window) {
      try {
        const isCurrentlyFullScreen =
          await window.electronAPI.window.isFullScreen();
    ...
    <>
      <motion.article className="settings-element">
        ...
        <div>
          ...
          <p>
            This card provides links to useful resources when you open the app
            for the first time.
          </p>
        </div>
      </motion.article>
        ...
        <div className="form-field-container">
          <div className="form-field">
            <select
              ...
              onChange={(e) => {
                setInterfaceScale(Number.parseInt(e.target.value, 10));
              }}
            >
      ...
      {!isCapacitor() && (
        <motion.article className="settings-element">
          ...
          <div>
            ...
            <p>
              Dynamic scaling lets Interviewer resize the user interface
              proportionally to the size of the window. Turning it off will use
              a fixed size.
            </p>
          </div>
        </motion.article>
      ...
      <motion.article className="settings-element">
        ...
        <div>
          ...
          <p>
            The full screen node form is optimized for smaller devices, or
            devices with no physical keyboard.
          </p>
        </div>
      </motion.article>
    </>
...
const mapDispatchToProps = (dispatch) => ({
  toggleSetting: (settingName) =>
    dispatch(deviceSettingsActions.toggleSetting(settingName)),
  setInterfaceScale: (scale) =>
    dispatch(deviceSettingsActions.setInterfaceScale(scale)),
});
