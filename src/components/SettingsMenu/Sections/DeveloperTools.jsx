...
import {
  getAdditionalAttributesForCurrentPrompt,
  getNodeEntryForCurrentPrompt,
    ...
    <>
      <motion.article className="settings-element">
        ...
        <div>
          ...
          <p>
            This feature adds interaction sounds to common actions in the app,
            which may improve the interview experience. These sounds were
            developed by our summer intern, Anika Wilsnack.
          </p>
        </div>
      </motion.article>
      <motion.article className="settings-element">
        ...
        <div>
          ...
          <p>
            This experimental features enables prompt text to be read aloud by
            the devices native text-to-speech (TTS) engine. This is an
            experimental feature that may not work correctly.
          </p>
        </div>
      </motion.article>
      <motion.article className="settings-element">
        ...
        <div>
          ...
          <p>
            Click the button above to reset all Interviewer data. This will
            erase any in-progress interviews, and all application settings.
          </p>
        </div>
      </motion.article>
      {shouldShowMocksItem && (
        <motion.article className="settings-element">
          <div className="form-field-container">
            <div className="form-field">
              <Button id="add-mock-nodes" onClick={handleAddMockNodes}>
                Add nodes
              </Button>
            </div>
          </div>
          <div>
            ...
            <p>
              During an active interview session, clicking this button will
              create mock nodes for testing purposes.
            </p>
          </div>
        </motion.article>
      )}
      <motion.article className="settings-element--sub-item">
        <div>
          ...
          <p>
            These protocols are used by the development team for testing
            purposes.
          </p>
        </div>
        <div className="form-field-container">
      ...
      <motion.article className="settings-element--sub-item">
        <div>
          ...
          <p>
            This function creates dummy data for testing export code and device
            performance.
          </p>
        </div>
        <h4>Use protocol</h4>
          ...
          <div className="form-field">
            ...
            >
              ...
              {Object.keys(installedProtocols).map((protocolId) => (
                <option value={protocolId} key={protocolId}>
...
const developerToolsHandlers = withHandlers({
  handleAddMockNodes: (props) => () => {
    ...
    props.generateMockNodes(
      nodeDefinition.variables,
      typeKey,
      20,
      props.additionalMockAttributes,
    );
    props.closeMenu();
  },
  handleResetAppData: (props) => () => {
    props.openDialog({
      ...
      message:
        'This will delete ALL data from Interviewer, including interview data and settings. Do you wish to continue?',
      onConfirm: () => {
        props.resetState();
      },
...
const mapDispatchToProps = (dispatch) => ({
  generateMockNodes: bindActionCreators(
    mockActions.generateMockNodes,
    dispatch,
  ),
  generateMockSessions: bindActionCreators(
    mockActions.generateMockSessions,
    dispatch,
  ),
  openDialog: bindActionCreators(dialogsActions.openDialog, dispatch),
  toggleSetting: (setting) =>
    dispatch(deviceSettingsActions.toggleSetting(setting)),
