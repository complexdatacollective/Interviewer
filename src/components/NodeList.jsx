...
class NodeList extends Component {
  ...
  UNSAFE_componentWillReceiveProps(newProps) {
    const { items, listId, disableDragNew } = this.props;
    ...
    if (
      isEqual(newProps.items, items) &&
      isEqual(newProps.disableDragNew, disableDragNew)
    ) {
      return;
    }
    ...
    this.setState({ exit: true }, () => {
      this.setState({ items: [], stagger: true }, () => {
  ...
  render() {
    ...
    const { stagger, items, exit } = this.state;

    const isSource = !!find(items, [
      entityPrimaryKeyProperty,
      get(meta, entityPrimaryKeyProperty, null),
    ]);
    ...
    return (
      <TransitionGroup className={classNames} style={styles} exit={exit}>
