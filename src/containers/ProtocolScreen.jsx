...
const childFactoryCreator = (stageBackward) => (child) =>
...
class Protocol extends Component {
  ...
  goToNext = (direction = 1) => {
    ...
    this.setState({ pendingStage: -1, pendingDirection: direction }, () =>
      beforeNext(direction),
    );
  };

  goToStage = (index) => {
    const { isSkipped, openDialog, goToStage } = this.props;
    if (isSkipped(index)) {
      openDialog({
        type: 'Warning',
  ...
  handleStageSelect = (index) => {
    const { stageIndex, stage } = this.props;
    ...
    this.setState({ pendingStage: index, pendingDirection: direction }, () =>
