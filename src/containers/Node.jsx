...
class Node extends PureComponent {
  ...
  initWorker(url) {
    ...
    const { nodeTypeDefinition, workerNetwork } = this.props;
  ...
  render() {
    const { color, workerUrl, getLabel } = this.props;
    const { workerError, label } = this.state;

    const useWorkerLabel = workerUrl !== false && !workerError;
    const dynamicLabel = useWorkerLabel ? label || '' : getLabel(this.props);
    return <UINode color={color} {...this.props} label={dynamicLabel} />;
  }
}
...
function mapStateToProps(state, props) {
  ...
  return {
    ...
    workerNetwork:
      (getNodeLabelWorkerUrl(state) && getWorkerNetwork(state)) || null,
  };
}
