...
describe('<PresetSwitcher />', () => {
  ...
  it('renders navigation', () => {
    ...
    expect(
      subject.dive().find('.preset-switcher__navigation--next'),
    ).toHaveLength(1);
  });
  ...
  it('handles navigation', () => {
    ...
    const subject = shallow(
      <PresetSwitcher {...props} onChangePreset={handler} />,
    );
  ...
  it('enables freeze button', () => {
    ...
    expect(handler).toHaveBeenCalled();
    expect(
      subject.dive().find('.preset-switcher__freeze--active'),
    ).toHaveLength(0);
  });

  it('activates freeze button', () => {
    const subject = shallow(
      <PresetSwitcher {...props} shouldShowFreezeButton isFrozen />,
    );
    expect(
      subject.dive().find('.preset-switcher__freeze--active'),
    ).toHaveLength(1);
  ...
  it('hides freeze button', () => {
    const subject = shallow(
      <PresetSwitcher {...props} shouldShowFreezeButton={false} />,
    );
