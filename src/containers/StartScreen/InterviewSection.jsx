...
const InterviewSection = () => {
  ...
  const lastActiveProtocol = useSelector((state) =>
    getLastActiveProtocol(state),
  );

  const dispatch = useDispatch();
  const addSession = (caseId, protocol) =>
    dispatch(sessionActions.addSession(caseId, protocol));
  const toggleUIOverlay = (overlay) => dispatch(uiActions.toggle(overlay));

  const showResumeSessionPicker = useSelector(
    (state) => state.ui.showResumeSessionPicker,
  );
  const showStartInterviewPicker = useSelector(
    (state) => state.ui.showStartInterviewPicker,
  );
              ...
              <div className="content-area">
                ...
                {Object.keys(installedProtocols).length > 1 && (
                  <div className="content-area__other">
                    ...
                    >
                      <h4>
                        {Object.keys(installedProtocols).length - 1 > 1
                          ? `+${Object.keys(installedProtocols).length - 1} Other Protocols`
                          : `+${Object.keys(installedProtocols).length - 1} Other Protocol`}
                      </h4>
                    </StackButton>
                  </div>
                )}
              ...
              <div className="content-area">
                ...
                {Object.keys(sessions).length > 1 && (
                  <div className="content-area__other">
                    ...
                    >
                      <h4>
                        {Object.keys(sessions).length - 1 > 1
                          ? `+${Object.keys(sessions).length - 1} Other Interviews`
                          : `+${Object.keys(sessions).length - 1} Other Interview`}
                      </h4>
                    </StackButton>
                  </div>
                )}
