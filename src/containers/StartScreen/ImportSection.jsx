...
const ImportSection = () => {
  ...
  const showProtocolUrlForm = useSelector(
    (state) => state.ui.showProtocolUrlForm,
  );
  const [showManageProtocolsOverlay, setShowManageProtocolsOverlay] =
    useState(false);

  const dispatch = useDispatch();
  const toggleShowProtocolUrlForm = () =>
    dispatch(uiActions.toggle('showProtocolUrlForm'));
    ...
    <Section className="start-screen-section import-section">
      ...
      {Object.keys(installedProtocols).length > 0 && (
        <footer className="import-section__manage-protocols">
          <Button
            color="platinum"
            onClick={() => setShowManageProtocolsOverlay(true)}
          >
            Manage Installed Protocols...
          </Button>
        </footer>
      )}
      <ProtocolUrlForm
        show={showProtocolUrlForm}
        handleClose={toggleShowProtocolUrlForm}
      />
