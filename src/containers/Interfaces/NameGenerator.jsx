...
const NameGenerator = (props) => {
  const { registerBeforeNext, prompt, stage } = props;

  const { prompts, form, quickAdd, behaviours, subject } = stage;
  ...
  const stageNodeCount = useSelector((state) =>
    makeGetStageNodeCount()(state, props),
  );
  const newNodeAttributes = useSelector((state) =>
    makeGetAdditionalAttributes()(state, props),
  );
  const newNodeModelData = useSelector((state) =>
    makeGetPromptNodeModelData()(state, props),
  );
  const nodesForPrompt = useSelector((state) =>
    makeNetworkNodesForPrompt()(state, props),
  );
  const nodeIconName = useSelector((state) =>
    makeGetNodeIconName()(state, props),
  );
  ...
  const addNode = (...properties) =>
    dispatch(sessionsActions.addNode(...properties));
  const addNodeToPrompt = (...properties) =>
    dispatch(sessionsActions.addNodeToPrompt(...properties));
  ...
  const handleBeforeLeaving = (direction, destination) => {
    const { isFirstPrompt, isLastPrompt, onComplete } = props;
