import { differenceBy, get, omit } from 'lodash';
...
import { CardList, FilterableListWrapper } from '../../components';
...
class NameGeneratorList extends Component {
  ...
  onSubmitNewNode = (node) => {
    const { newNodeAttributes, newNodeModelData, addNode } = this.props;
    const attributeData = {
      ...newNodeAttributes,
  ...
  isNodeSelected = (node) => {
    ...
    return selectedNodes.some(
      (current) =>
        current[entityPrimaryKeyProperty] === node[entityPrimaryKeyProperty],
    );
  };
  ...
  detailsWithVariableUUIDs = (node) => {
    const { nodeTypeDefinition, visibleSupplementaryFields } = this.props;
