...
export const getNetwork = createDeepEqualSelector(
  ...
  (session) => session?.network || { nodes: [], edges: [], ego: {} },
);
...
export const makeGetNodeTypeDefinition = () =>
  createDeepEqualSelector(
    (state, props) => getProtocolCodebook(state, props),
    (state, props) =>
      get(props, 'type') || // When used in <Node /> // TODO: should use makeGetSubject
...
export const makeGetNodeLabel = () =>
  createDeepEqualSelector(
    ...
    (nodeTypeDefinition) => (node) =>
      labelLogic(nodeTypeDefinition, getEntityAttributes(node)),
  );
