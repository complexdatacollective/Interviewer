...
const closeFriendNode = {
  uid: 2,
  attributes: { name: 'bar', type: 'person', close_friend: true },
};
...
describe('name generator selector', () => {
  describe('makeGetPanelConfiguration()', () => {
    ...
    it('always returns an array', () => {
      const subject = getPanelConfiguration(mockState, {
        stage: {},
      });
  ...
  describe('memoed selectors', () => {
    it('should get card additional properties', () => {
      expect(NameGen.getCardAdditionalProperties(mockState, mockProps)).toEqual(
        ['blue'],
      );
    ...
    it('should get sort defaults', () => {
      expect(NameGen.getInitialSortOrder(mockState, mockProps)).toEqual([
