import { findKey, get, omit } from 'lodash';
...
const protocolHasSessions = (state, protocolUID) =>
  new Promise((resolve) => {
    ...
    const protocolName = get(
      state,
      ['installedProtocols', protocolUID, 'name'],
      null,
    );
...
const hasNonExportedSessionDialog = (protocolName) => ({
  ...
  message: (
    <>
      ...
      <p>
        <strong>Deleting this protocol will also delete these sessions.</strong>
      </p>
    </>
  ),
  ...
  protocolHasSessions(getState(), protocolUID)
    .then(({ hasSession, hasNotExportedSession, protocolName }) => {
      if (hasNotExportedSession) {
        return dispatch(
          dialogActions.openDialog(hasNonExportedSessionDialog(protocolName)),
        );
      }

      if (hasSession) {
        return dispatch(
          dialogActions.openDialog(hasSessionDialog(protocolName)),
        );
      }
    ...
    .then((confirmed) => {
      ...
      deleteProtocol(protocolUID).then(() =>
