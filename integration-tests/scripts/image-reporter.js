...
class ImageReporter {
  ...
  onTestResult(test, testResult, aggregateResults) {
    if (testResult.numFailingTests && testResult.failureMessage.match(/different from snapshot/)) {
      const files = fs.readdirSync(`${process.rootDir}/__tests__/__image_snapshots__/__diff_output__/`);
      files.forEach((value) => {
        console.log('value', value);
        imgbbUploader(IMGBB_API_KEY, `${process.rootDir}/__tests__/__image_snapshots__/__diff_output__/${value}`)
          .then(response => {
