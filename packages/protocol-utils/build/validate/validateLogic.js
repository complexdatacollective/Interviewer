...
import { duplicateId, duplicateInArray, entityDefFromRule, getVariablesForSubject, getVariableNames, getEntityNames, nodeVarsIncludeDisplayVar, getVariableNameFromID, getSubjectTypeName, } from './helpers.js';
/**
...
const validateLogic = (protocol) => {
    ...
    v.addValidation('codebook', codebook => !duplicateInArray(getEntityNames(codebook)), codebook => `Duplicate entity name "${duplicateInArray(getEntityNames(codebook))}"`);
    v.addValidation('codebook.node.*', nodeType => nodeVarsIncludeDisplayVar(nodeType), nodeType => `node displayVariable "${nodeType.displayVariable}" did not match any node variable`);
    ...
    validation => !Object.keys(validation).includes('unique'), (_, __, keypath) => `The 'unique' variable validation cannot be used on ego variables. Was used on ego variable "${getVariableNameFromID(codebook, { entity: 'ego' }, keypath[4])}".`);
    ...
    (validations, _, keypath) => {
        ...
        const typesToCheck = Object.keys(validations).filter(validation => typesWithVariables.includes(validation));
    ...
    v.addValidation('protocol.stages', stages => !duplicateId(stages), stages => `Stages contain duplicate ID "${duplicateId(stages)}"`);
    v.addValidation('stages[].panels', panels => !duplicateId(panels), panels => `Panels contain duplicate ID "${duplicateId(panels)}"`);
    v.addValidation('.rules', rules => !duplicateId(rules), rules => `Rules contain duplicate ID "${duplicateId(rules)}"`);
    v.addValidation('stages[].prompts', prompts => !duplicateId(prompts), prompts => `Prompts contain duplicate ID "${duplicateId(prompts)}"`);
    v.addValidation('stages[].items', items => !duplicateId(items), items => `Items contain duplicate ID "${duplicateId(items)}"`);
    v.addValidation('codebook.*.*.variables', variableMap => !duplicateInArray(getVariableNames(variableMap)), variableMap => `Duplicate variable name "${duplicateInArray(getVariableNames(variableMap))}"`);
    ...
    v.addValidation('prompts[].variable', (variable, subject) => getVariablesForSubject(codebook, subject)[variable], (variable, subject) => `"${variable}" not defined in codebook[${subject.entity}][${subject.type}].variables`);
    // 'otherVariable' is used by categorical bin for 'other' responses. Check this variable
    // exists in the stage subject codebook
    v.addValidation('prompts[].otherVariable', (otherVariable, subject) => getVariablesForSubject(codebook, subject)[otherVariable], (otherVariable, subject) => `"${otherVariable}" not defined in codebook[${subject.entity}][${subject.type}].variables`);
    ...
    v.addValidation('prompts[].createEdge', createEdge => Object.keys(codebook.edge).includes(createEdge), createEdge => `"${createEdge}" definition for createEdge not found in codebook["edge"]`);
    ...
    // is a variable ID.
    v.addValidation('prompts[].layout.layoutVariable', (variable, subject) => {
    ...
    v.addValidation('prompts[].additionalAttributes', (additionalAttributes, subject) => additionalAttributes.every(({ variable }) => getVariablesForSubject(codebook, subject)[variable]), additionalAttributes => `One or more sortable properties not defined in codebook: ${additionalAttributes.map(({ variable }) => variable)}`);
    ...
    v.addValidation('prompts[].edges.restrict.origin', origin => Object.keys(codebook.node).includes(origin), origin => `"${origin}" is not a valid node type.`);
    v.runValidations();
    v.warnings.forEach(warning => console.error(warning)); // eslint-disable-line no-console
    return v.errors;
};
export default validateLogic;
//# sourceMappingURL=validateLogic.js.map