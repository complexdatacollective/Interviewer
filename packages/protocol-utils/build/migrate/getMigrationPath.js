...
const isMigrationPathValid = path => !path.some(({ migration }) => !migration);
const matchMigrations = (sourceVersion, targetVersion) => ({ version }) => version > sourceVersion && version <= targetVersion;
const getMigrationPath = (rawSourceSchemaVersion, targetSchemaVersion) => {
    ...
    const migrationPath = migrations.filter(matchMigrations(sourceSchemaVersion, targetSchemaVersion));
...
//# sourceMappingURL=getMigrationPath.js.map