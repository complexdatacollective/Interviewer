 ...
 */
const setProps = (props, source = {}) => Object.keys(props)
    .reduce((acc, key) => {
    if (!source[key]) {
        return acc;
    }
...
const getNames = (obj = {}) => Object.keys(obj)
    .map(key => obj[key].name);
const migrateOptionValues = (options = []) => options.reduce((acc, { value, ...rest }) => ([
    ...acc,
...
const migrateVariables = (variables = {}) => Object.keys(variables)
...
const migrateTypes = (types = {}) => Object.keys(types)
    .reduce((acc, typeId) => ({
    ...acc,
    [typeId]: migrateType(types[typeId], acc),
}), {});
const migratePrompt = (prompt = {}) => {
    const booleanOnlyAttributes = prompt.additionalAttributes.filter(additionalAttribute => additionalAttribute.value === true || additionalAttribute.value === false);
...
const migrateStages = (stages = []) => stages.map(stage => stage.prompts ? migrateStage(stage) : stage);
